...
less than 5 with the keyword _RARE_, and produce counts in rare_replace.dat file.
"""

def read_and_rare_replace(fname):
    """
    If Count(word) < 5, replace it with the keyword _RARE_.
    """
    try:
        infile = open(fname, "r")
    except IOError:
    ...
    outfile = open("rare_replaced.dat", "w")
...
def usage():
    ...
    python count_freqs.py [input_file]
        Read in a named entity tagged training input file, replace any words with a count
        less than 5 with the keyword _RARE_, and produce counts in rare_replace.dat
    """
...
if __name__ == "__main__":
    ...
    read_and_rare_replace(sys.argv[1])
